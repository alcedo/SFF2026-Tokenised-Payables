...
**Control skill.** Pick it by surface. Browser, Electron, web UIs, CLIs, and TUIs all go through Claude Code's `run` skill. Native mobile uses whatever simulator-driving skill the repo has. A PR that touches two surfaces gets lanes on both. A surface with no control skill is a risk in Appendix C, and its live block still names how each lane drives it.
