...
At the end of the run, before handing back, check the log told the truth. Read this run's transcript under this workspace's transcript directory, `~/.claude/projects/<slug>/`, where `<slug>` is the absolute working directory with every `/` replaced by `-` (so `/home/user/proj` becomes `-home-user-proj`). Don't glob across `~/.claude/projects/*/`. That reads unrelated private chats. Walk the log against what actually happened:
