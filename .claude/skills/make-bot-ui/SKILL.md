...
> **Not ported to Claude Code.** This skill drives Cursor's automations webhook API
> (`https://api2.cursor.sh/automations/webhook/<id>`), which has no Claude Code equivalent.
> The closest analogue is a Claude Code Routine (`create_trigger` / `fire_trigger` on the
> Claude Code Remote MCP server), whose API is different enough that the steps below do not
> transfer. Kept for reference; read it as Cursor documentation, not as instructions.
