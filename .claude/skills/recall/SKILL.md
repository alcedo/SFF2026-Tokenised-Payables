...
Transcripts live at `~/.claude/projects/<slug>/<session-uuid>.jsonl`, where `<slug>` is the absolute workspace path with every "/" turned into "-", leading slash included (so `/Users/you/proj` becomes `-Users-you-proj`). Subagent transcripts sit one level deeper, under `~/.claude/projects/<slug>/<session-uuid>/`. Every line is one chat message.
